...
public class StdevSPARQLFunctionSymbolImpl extends UnaryNumericSPARQLAggregationFunctionSymbolImpl {
    ...
    private StdevSPARQLFunctionSymbolImpl(RDFTermType rootRdfTermType, boolean isPop, boolean isDistinct, boolean shortName) {
        super("SP_STDEV"+ (!shortName ? (isPop ? "_POP" : "_SAMPLE") : "") + (isDistinct ? "_DISTINCT" : ""), shortName ? AGG.STDEV.getIRIString() : (isPop ? AGG.STDEV_POP.getIRIString() : AGG.STDEV_SAMP.getIRIString()), isDistinct, rootRdfTermType, DEFAULT_AGG_VAR_NAME);
        this.isPop = isPop;
    }
    ...
    protected ImmutableFunctionalTerm createAggregate(ConcreteNumericRDFDatatype rdfType, ImmutableTerm dbTerm,
        ...
        return termFactory.getDBStdev(dbTerm, dbType, isPop, isDistinct());
    }
