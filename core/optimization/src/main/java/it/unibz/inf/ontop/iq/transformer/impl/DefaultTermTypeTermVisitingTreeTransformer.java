...
public class DefaultTermTypeTermVisitingTreeTransformer
    ...
    private IQTree replaceTypeTermConstants(IQTree child) {
        return Optional.of(child.getRootNode())
                ...
                .filter(s -> !s.isEmpty())
                .map(substitution -> substitutionFactory.transform(substitution, this::replaceTypeTermConstants))
    ...
    private IQTree enforceUsageOfCommonTypeFunctionSymbol(IQTree tree,
        ...
        ImmutableSubstitution<ImmutableTerm> newSubstitution = substitutionFactory.transform(initialConstructionNode.getSubstitution(),
