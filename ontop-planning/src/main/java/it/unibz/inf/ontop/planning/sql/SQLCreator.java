...
public class SQLCreator {
	...
	public String joinString() {
	    ...
	    //  FIXME: I termini sono 2 !!! Uno per ogni frammento. 
		...
		for( int varIndex = 0; varIndex < t.getArity(); ++varIndex ){
			...
			String colName = JoinStructurer.FRAG_ID + fragIndex + "." + term.getProjNameForTermVariable(termVar);
	...
	class LocalUtils{ // Helper class
		...
		for( String cq : splits ){
			...
			for( int termVarCounter = 0; termVarCounter < t.getTermVariables().size(); ++termVarCounter ){
			    ...
			    String newProjElement = commaSplits.get(splitCounter++) + " AS " + projName; // BUG! Term 1, var 0 = Term 0 
	...
	JoinStructurerOLD structurer = new JoinStructurerOLD(); // Davide> At the moment I am re-creating this every time
	//       but this behavior could be optimized I think...
