...
WHERE ((POSITION('banded' IN v1."label") > 0) AND v1."nr" IS NOT NULL AND v1."label" IS NOT NULL)
)UNION ALL 
(SELECT v3."label" AS "label10m40", v3."nr" AS "nr0m49"
FROM "bsbm"."product2" v3
WHERE ((POSITION('banded' IN v3."label") > 0) AND v3."nr" IS NOT NULL AND v3."label" IS NOT NULL)
)) v5
