...
public class MappingSplitter {
		...
		for (OBDAMappingAxiom mapping : mappings) {
			...
			if(bodyAtoms.size() == 1){
				...
				for (Function bodyAtom : bodyAtoms) {
					String newId = String.format("%s##%d", id, counter++);
	...
	/**
	 * this method split the mappings in {@link obdaModel} with sourceId 
	 * 
	 * @param obdaModel
	 * @param sourceId
	 */
	public void splitMappings(OBDAModel obdaModel, URI sourceId) {
		List<OBDAMappingAxiom> splittedMappings = splitMappings(obdaModel.getMappings(sourceId));
