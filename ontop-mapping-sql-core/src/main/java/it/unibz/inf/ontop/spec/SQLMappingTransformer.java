...
public class SQLMappingTransformer implements MappingTransformer {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(SQLMappingTransformer.class);
    ...
    @Inject
    public SQLMappingTransformer(Mapping2DatalogConverter mapping2DatalogConverter, Datalog2QueryMappingConverter mappingConverter, OntopMappingSQLSettings settings, MappingNormalizer mappingNormalizer, SpecificationFactory specificationFactory, TMappingExclusionConfig tMappingExclusionConfig) {
    ...
    public OBDASpecification transform(Mapping mapping, RDBMetadata dbMetadata, Ontology ontology, TBoxReasoner tBox, ExecutorRegistry executorRegistry)
        ...
        ImmutableList<CQIE> mappingRulesWithFacts = insertFacts(fullyTypedRules, ontology,
                mapping.getMetadata().getUriTemplateMatcher());
        ...
        Mapping saturatedMapping = mappingConverter.convertMappingRules(saturatedMappingRules, dbMetadata,
                executorRegistry, mapping.getMetadata());
...
*/
    ...
    @Override
    public OBDASpecification transform(Mapping mapping, DBMetadata dbMetadata, Ontology ontology, TBoxReasoner tBox, ExecutorRegistry executorRegistry) throws MappingException, OntologyException {
        return null;
    }
}
