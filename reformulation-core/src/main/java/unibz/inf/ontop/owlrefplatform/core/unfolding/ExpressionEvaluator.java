...
import unibz.inf.ontop.owlrefplatform.core.basicoperations.SubstitutionImpl;
import unibz.inf.ontop.owlrefplatform.core.basicoperations.UnifierUtilities;
import unibz.inf.ontop.owlrefplatform.core.basicoperations.UriTemplateMatcher;
import unibz.inf.ontop.model.impl.TermUtils;
import unibz.inf.ontop.owlrefplatform.core.basicoperations.SubstitutionUtilities;
...
public class ExpressionEvaluator {
				...
				if (old instanceof Function) {
					...
					if (!newterm.equals(old))
						if (newterm == OBDAVocabulary.FALSE) {
							//
							terms.set(i, fac.getTypedTerm(OBDAVocabulary.FALSE, Predicate.COL_TYPE.BOOLEAN));
			...
			if (predicate instanceof DatatypePredicate) {
				if (dtfac.isLiteral(predicate)) { // R: was datatype.equals(OBDAVocabulary.RDFS_LITERAL_URI)
					return fac.getTypedTerm(
							fac.getVariable(parameter.toString()), Predicate.COL_TYPE.LITERAL);
				} 
					...
					return fac.getTypedTerm(
							fac.getFunctionCast(fac.getVariable(parameter.toString()),
									fac.getConstantLiteral(dtfac.getDatatypeURI(Predicate.COL_TYPE.LITERAL).stringValue())),
										Predicate.COL_TYPE.LITERAL);
				}
			} 
			else if (predicate instanceof URITemplatePredicate) {
				return fac.getTypedTerm(function.clone(), Predicate.COL_TYPE.LITERAL);
			} 
	...
	private Term getDatatype(Predicate predicate, Term lit)
	{
		if (predicate instanceof DatatypePredicate) {
			...
			return fac.getUriTemplateForDatatype(dtfac.getDatatypeURI(Predicate.COL_TYPE.BOOLEAN).stringValue());
		} 
	...
	private Predicate getDatatypePredicate(Term term) {
		if (term instanceof Function) {
			...
			if (pred == null)
				pred = dtfac.getTypePredicate(Predicate.COL_TYPE.STRING); // .XSD_STRING;
			return pred;
		} 
		else {
	...
	private boolean isDouble(Predicate pred) {
		return (pred.equals(dtfac.getTypePredicate(Predicate.COL_TYPE.DOUBLE)) || pred.equals(dtfac.getTypePredicate(Predicate.COL_TYPE.FLOAT)));
	}
	...
	private Term evalLang(Function term) {
		...
		Term emptyconstant = fac.getTypedTerm(fac.getConstantLiteral("", Predicate.COL_TYPE.LITERAL), Predicate.COL_TYPE.LITERAL);
		...
		if (function.getTerms().size() != 2) {
			...
			if (parameter instanceof Variable) {
				return fac.getTypedTerm(parameter.clone(), Predicate.COL_TYPE.LITERAL);
			}
			else if (parameter instanceof Constant) {
				return fac.getTypedTerm(
						fac.getConstantLiteral(((Constant) parameter).getValue(), Predicate.COL_TYPE.LITERAL), Predicate.COL_TYPE.LITERAL);
			}
		}
		return term;
	}
	...
	public Term evalEqNeq(Function term, boolean eq) {
		...
		if (eval1 instanceof Variable || eval2 instanceof Variable) {
			...
			if (pred1 instanceof DatatypePredicate) {
				if (pred1.getType(0) == Predicate.COL_TYPE.UNSUPPORTED) {
			...
			if (eval2 instanceof Function) {
				...
				if (pred2.getType(0) == Predicate.COL_TYPE.UNSUPPORTED) {
