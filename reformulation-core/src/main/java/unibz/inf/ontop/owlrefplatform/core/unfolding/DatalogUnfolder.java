...
import unibz.inf.ontop.model.impl.VariableImpl;
...
import unibz.inf.ontop.owlrefplatform.core.basicoperations.EQNormalizer;
...
import static unibz.inf.ontop.model.impl.DatalogTools.isLeftJoinOrJoinAtom;
		...
		for (Function atom : innerAtoms) {
			if (atom.isDataFunction() || atom.isAlgebraFunction()) {
				...
				for (Variable var: variablesArg2){
					...
					containsVar = referencedVariables.contains(var);
					if (containsVar){
