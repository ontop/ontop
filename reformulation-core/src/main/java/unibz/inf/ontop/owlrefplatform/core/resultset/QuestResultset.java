package unibz.inf.ontop.owlrefplatform.core.resultset;
...
public class QuestResultset implements TupleResultSet {
	...
	public Constant getConstant(int column) throws OBDAException {
		...
		try {
			...
			Predicate.COL_TYPE type = getQuestType((byte) set.getInt(column - 2));

			if (type == Predicate.COL_TYPE.NULL || realValue == null) {
				return null;
			} else {
				if (type == Predicate.COL_TYPE.OBJECT) {
				...
				} else if (type == Predicate.COL_TYPE.BNODE) {
					 ...
					 * properly.
					 */
					if (type == Predicate.COL_TYPE.LITERAL) {
					...
					} else if (type == Predicate.COL_TYPE.BOOLEAN) {
						boolean value = set.getBoolean(column);
						result = fac.getBooleanConstant(value);
					} else if (type == Predicate.COL_TYPE.DOUBLE) {
						double d = set.getDouble(column);
						// format name into correct double representation

						String s = formatter.format(d);
						result = fac.getConstantLiteral(s, Predicate.COL_TYPE.DOUBLE);

					} else if (type == Predicate.COL_TYPE.DATETIME) {
                    ...
                    try {
					 ...
					 else if (type == Predicate.COL_TYPE.DATETIME_STAMP) {
					...
					else if (type == Predicate.COL_TYPE.DATE) {
						if (!isOracle) {
							Date value = set.getDate(column);
							result = fac.getConstantLiteral(value.toString(), Predicate.COL_TYPE.DATE);
						} 
						else {
							String value = set.getString(column);
                    ...
                    else if (type == Predicate.COL_TYPE.TIME) {
						Time value = set.getTime(column);						
						result = fac.getConstantLiteral(value.toString().replace(' ', 'T'), Predicate.COL_TYPE.TIME);
					 
					} else if (type == Predicate.COL_TYPE.INTEGER) {
	...
	private Predicate.COL_TYPE getQuestType(int typeCode) {

        Predicate.COL_TYPE questType = Predicate.COL_TYPE.getQuestType(typeCode);
