package unibz.inf.ontop.owlrefplatform.core.translator;
...
public class SparqlAlgebraToDatalogTranslator {
        ...
        if ((p != null) && p.toString().equals(RDF.TYPE.stringValue())) {
            ...
            if (o == null) {
                ...
                if (type != null) {
                    ...
                    Predicate.COL_TYPE subjectType = null; // are never changed
                    Predicate predicate = ofac.getPredicate(objectUri.stringValue(), new Predicate.COL_TYPE[] { subjectType });
                    return ofac.getFunction(predicate, sTerm);
                }
            }
            else
            ...
            if (p != null) {
                Predicate.COL_TYPE subjectType = null; // are never changed
                Predicate.COL_TYPE objectType = null;
                Predicate predicate = ofac.getPredicate(p.stringValue(), new Predicate.COL_TYPE[] { subjectType, objectType });
    ...
    private Term getOntopTerm(Var subj) {
        ...
        if (s == null) {
            ...
            Predicate.COL_TYPE objectType;
            if (type == null)
                objectType = Predicate.COL_TYPE.LITERAL;
            else {
            ...
            if ((objectType == Predicate.COL_TYPE.DECIMAL) && !value.contains("."))  {
                // put the type as integer (decimal without fractions)
                objectType = Predicate.COL_TYPE.INTEGER;
            }

            unibz.inf.ontop.model.ValueConstant constant = ofac.getConstantLiteral(value, objectType);

            // v1.7: We extend the syntax such that the data type of a
            // constant is defined using a functional symbol.
            if (objectType == Predicate.COL_TYPE.LITERAL) {
            ...
            if (uriRef != null) {
                ...
                result = ofac.getUriTemplate(ofac.getConstantLiteral(String.valueOf(id), Predicate.COL_TYPE.INTEGER));
            }
            else {
    ...
    private Term getConstantExpression(Value v) {
        ...
        if (v instanceof Literal) {
            ...
            Predicate.COL_TYPE tp;
            if (type == null) {
                tp = Predicate.COL_TYPE.LITERAL;
            }
            else {
            ...
            unibz.inf.ontop.model.ValueConstant constant = ofac.getConstantLiteral(constantString, tp);
