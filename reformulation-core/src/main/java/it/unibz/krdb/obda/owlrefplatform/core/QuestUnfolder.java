...
public class QuestUnfolder {
	...
	public QuestUnfolder(OBDAModel unfoldingOBDAModel, DBMetadata metadata, Connection localConnection, URI sourceId, TMappingExclusionConfig excludeFromTMappings)  throws Exception{
		 ...
		 */
		List<OBDAMappingAxiom> splittedMappings = MappingSplitter.splitMappings(mappings);
		...
		List<OBDAMappingAxiom> expandedMappings = metaMappingExpander.expand(splittedMappings);
		...
		List<OBDAMappingAxiom> finalMappings = unfoldingOBDAModel.getMappings(sourceId);
		unfoldingProgram = Mapping2DatalogConverter.constructDatalogProgram(finalMappings, metadata);
...
//		log.debug("Appended {} ABox assertions as fact rules", count);		
	 ...
	 */
	private static void preprocessProjection(List<OBDAMappingAxiom> mappings, DBMetadata metadata) throws SQLException {
