...
public class SQLGenerator implements SQLQueryGenerator {
	...
	private String getMainColumnForSELECT(Term ht, List<String> signature,
			int hpos, QueryAliasIndex index) {
	...
	private String getLangColumnForSELECT(Term ht, List<String> signature,
			int hpos, QueryAliasIndex index) {
	...
	private String getTypeColumnForSELECT(Term ht, List<String> signature,
			int hpos) {
	...
	public class QueryAliasIndex {
		...
		private void indexVariables(Function atom) {
			...
			for (int index = 0; index < atom.getTerms().size(); index++) {
				...
				String reference = sqladapter.sqlQualifiedColumn(
						viewName, columnName);
