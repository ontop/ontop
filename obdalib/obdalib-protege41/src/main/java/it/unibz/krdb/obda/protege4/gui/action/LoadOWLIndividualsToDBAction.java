...
public class LoadOWLIndividualsToDBAction extends ProtegeAction {
	...
	private void loadAboxToDB() {
		...
		Runnable showdialog = new Runnable() {
			public void run() {
				OBDAModelManager controller = (OBDAModelManager)getEditorKit().get(OBDAModelImpl.class.getName());
				...
				Thread th = new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							OWLAPI3ABoxIterator aboxiterator = new OWLAPI3ABoxIterator(ontologies);
