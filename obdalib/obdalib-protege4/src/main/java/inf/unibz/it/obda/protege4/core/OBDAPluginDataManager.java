 ...
 * prefixes and if so they are loaded and administered by the prefix manager.
 *
...
public class OBDAPluginDataManager extends DataManager {
	...
	private void fillPrefixManagerWithDefaultValues(){
		...
		prefixManager.addUri(CURRENT_OBDA_FILE_VERSION_MAJOR + "." + CURRENT_OBDA_FILE_VERSION_MINOR, "version");
		prefixManager.addUri(ontoUrl, "xml:base");
		prefixManager.addUri(ontoUrl, "xmlns");
		prefixManager.addUri("http://www.w3.org/2006/12/owl2-xml#", "owl2xml");
		...
		while(sit.hasNext()){
			...
			if(!(key.equals("dc") || key.equals("dcterms")|| key.equals("dctype")||
				...
				prefixManager.addUri(mapper.getValue(key), key);
			}
		}
	}
