...
public class SQLGeneratorNonNested implements SourceQueryGenerator {
	...
	private static final org.slf4j.Logger log = LoggerFactory.getLogger(SQLGeneratorNonNested.class);

	public SQLGeneratorNonNested(DBMetadata metadata, JDBCUtility jdbcutil) {
				...
				if (ht instanceof Variable) {
					...
					if (functionSymbol instanceof DataTypePredicate) {
						...
						if (functionString.equals("http://obda.org/quest#uri")) {
							...
							sb.append(" AS ");
							sb.append(jdbcutil.quote(signature.get(hpos) + termidx));
							
							for (termidx = 1; termidx < terms.size(); termidx++) {

								sb.append(", ");

								Term functionTerm = terms.get(termidx);
								String sqlstring = "";
								if (functionTerm instanceof ValueConstant) {
									sqlstring = jdbcutil.getSQLLexicalForm((ValueConstant) functionTerm);
								} else {
									sqlstring = getSQLString(functionTerm, body, tableName, viewName, varAtomIndex, varAtomTermIndex, false);
								}
								sb.append(sqlstring);
								sb.append(" AS ");
								sb.append(jdbcutil.quote(signature.get(hpos) + termidx));
							...
							/*
							 * Cannot construct URI's if not using the URI 'official' uri function
							 */
							throw new OBDAException("Impossible to generate object  URIs using the given fuction. Function: " + functionString);
