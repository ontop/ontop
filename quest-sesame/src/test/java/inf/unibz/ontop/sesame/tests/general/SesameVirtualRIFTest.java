...
public class SesameVirtualRIFTest extends TestCase {
	...
	public void test() throws Exception
	{
		...
		try {
			...
			String obdafile = "/home/constant/gisat/gisat-geosparql.obda";
			...
			///query repo
			 try {
				 ...
				 String qualitative = prefixes + "SELECT  distinct ?partCD ?cdgeo   \n"
					 		+ "WHERE { \n"
					 		//+ "?x1 geosparql:asWKT ?uageo . \n" 
					 		+"?partCD rdf:type <http://melodiesproject.eu/CityDistricts/ontology#CityDistrict>  .\n"
					 		+ "?x1 rdf:type <http://geo.linkedopendata.gr/urban/ontology#prahaArea> .\n" 
					 		+ "?partCD geosparql:hasGeometry ?cdgeom . \n"
					 		+ "?cdgeom geosparql:asWKT ?cdgeo . \n"
					 	//	+ "?x1 geosparql:asWKT ?uageo . \n" 
					 		+ "?partCD <http://www.opengis.net/ont/geosparql#sfContains> ?x1 .\n"
	 						//+"FILTER(<http://www.opengis.net/def/function/geosparql/sfContains>(?cdgeo,?uageo)) \n" 
					 		+ "} limit 5";
					 
				 String quantitative = prefixes + "SELECT distinct ?partCD   \n"
					 		+ "WHERE { \n"
					 		+ "?x1 geosparql:asWKT ?uageo . \n" 
					 	//	+ "?x1 rdf:type ua:prahaArea .\n" 
					 //		+"?partCD rdf:type ?type . \n" 
				 		+ "?partCD geosparql:hasGeometry ?cdgeom . \n"
					 		+ "?cdgeom geosparql:asWKT ?cdgeo . \n"
	 						+"FILTER(<http://www.opengis.net/def/function/geosparql/sfContains>(?cdgeo,?uageo)) \n" 
					 		+ "} limit 5";
					 
			      TupleQuery tupleQuery = con.prepareTupleQuery(QueryLanguage.SPARQL, qualitative);
