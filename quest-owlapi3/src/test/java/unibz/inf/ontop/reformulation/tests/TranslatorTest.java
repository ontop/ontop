...
import unibz.inf.ontop.ontology.BinaryAxiom;
...
public class TranslatorTest extends TestCase {
	...
	public void test_3() throws Exception{
		...
		Assert.assertEquals("http://example/R", included.getPredicate().getName().toString());
		
		ObjectPropertyExpression indlucing = a.getSuper();
		assertEquals(true, indlucing.isInverse());
		Assert.assertEquals("http://example/S", indlucing.getPredicate().getName().toString());
		
		included = b.getSub();
		assertEquals(false, included.isInverse());
		Assert.assertEquals("http://example/S", included.getPredicate().getName().toString());
		
		indlucing = b.getSuper();
		assertEquals(true, indlucing.isInverse());
		Assert.assertEquals("http://example/R", indlucing.getPredicate().getName().toString());
	}
	...
	public void test_4() throws Exception{
		...
		Assert.assertEquals("http://example/A", included.getPredicate().getName().toString());
		
		OClass indlucing = (OClass) c1.getSuper();
		Assert.assertEquals("http://example/B", indlucing.getPredicate().getName().toString());
		
		included = (OClass) c2.getSub();
		Assert.assertEquals("http://example/B", included.getPredicate().getName().toString());
		
		indlucing = (OClass) c2.getSuper();
		Assert.assertEquals("http://example/A", indlucing.getPredicate().getName().toString());
	}
}
